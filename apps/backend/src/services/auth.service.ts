...
import type { JWTPayload, ServiceResult } from "../types/common"
...
export interface IAuthService {
  ...
  changePassword(userId: string, data: ChangePasswordDto): Promise<ServiceResult<void>>
...
export class AuthService implements IAuthService {
  ...
  async register(data: RegisterDto): Promise<ServiceResult<AuthResponseDto>> {
    try {
      ...
      const existingUser = await this.userRepository.exists(data.email, data.username)
      if (existingUser) {
        return {
          success: false,
  ...
  async changePassword(userId: string, data: ChangePasswordDto): Promise<ServiceResult<void>> {
    try {
  ...
  async generateTokens(user: User): Promise<{ token: string; expiresIn: number }> {
    try {
