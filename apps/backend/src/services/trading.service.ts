...
import { type IOrderRepository, OrderRepository } from "../repositories/order.repository"
import { type IUserRepository, UserRepository } from "../repositories/user.repository"
import type { OrderWithRelations, ServiceResult, TradeWithRelations } from "../types/common"
import { AppError } from "../types/common"

export interface ITradingService {
  createOrder(userId: string, data: CreateOrderDto): Promise<ServiceResult<Orders>>
  updateOrder(userId: string, orderId: string, data: UpdateOrderDto): Promise<ServiceResult<Orders>>
...
export class TradingService implements ITradingService {
  ...
  async createOrder(userId: string, data: CreateOrderDto): Promise<ServiceResult<Orders>> {
    try {
  ...
  async cancelOrder(userId: string, orderId: string): Promise<ServiceResult<Orders>> {
    try {
  ...
  async getOrderById(orderId: string): Promise<ServiceResult<OrderWithRelations>> {
    try {
  ...
  async getUserTrades(
    ...
    try {
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : "Failed to get user trades",
      }
    }
  }
    ...
    try {
      ...
      if (order.type === OrderType.MARKET) {
        ...
        if (trades.length > 0) {
          ...
          const totalExecuted = trades.reduce((sum, trade) => sum + Number(trade.amount), 0)
          const status =
  ...
  async matchOrders(newOrder: Orders): Promise<Trade[]> {
    try {
      ...
      for (const matchingOrder of matchingOrders) {
        ...
        const tradeAmount = Math.min(remainingAmount, Number(matchingOrder.amount))
        const tradePrice = Number(matchingOrder.price) || Number(newOrder.price) || 0

        // Create trade record
        const trade = await prisma.trade.create({
          data: {
            buyOrderId: newOrder.side === "BUY" ? newOrder.id : matchingOrder.id,
            sellOrderId: newOrder.side === "SELL" ? newOrder.id : matchingOrder.id,
            buyUserId: newOrder.side === "BUY" ? newOrder.userId : matchingOrder.userId,
            sellUserId: newOrder.side === "SELL" ? newOrder.userId : matchingOrder.userId,
        ...
        const matchingOrderRemainingAmount = Number(matchingOrder.amount) - tradeAmount
        if (matchingOrderRemainingAmount <= 0) {
          await this.orderRepository.updateStatus(matchingOrder.id, OrderStatus.FILLED)
        } else {
          await this.orderRepository.updateStatus(matchingOrder.id, OrderStatus.PARTIALLY_FILLED)
