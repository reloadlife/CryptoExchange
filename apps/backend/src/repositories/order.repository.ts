...
import type { CreateOrderDto, OrderQueryDto, UpdateOrderDto } from "../models/dto/trading.dto"
...
export interface IOrderRepository {
  ...
  findMany(params: OrderQueryDto): Promise<{ data: OrderWithRelations[]; meta: any }>
...
export class OrderRepository extends BaseRepositoryImpl<Orders> implements IOrderRepository {
  ...
  async findMany(params: OrderQueryDto): Promise<{ data: OrderWithRelations[]; meta: any }> {
    try {
      const { status, type, side, currencyId, sortBy, sortOrder } = params
      const { skip, take, page, limit } = this.buildPagination(params)
